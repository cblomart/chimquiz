...
118;Og;Oganesson;294.0";
    ...
    private static IReadOnlyList<Element> ParseElements()
    {
        ...
        foreach (var line in CsvData.Split('\n', StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            if (!int.TryParse(parts[0], out var atomicNumber)) continue;
            if (!decimal.TryParse(parts[3], System.Globalization.NumberStyles.Any,
                System.Globalization.CultureInfo.InvariantCulture, out var mass)) continue;

            var symbol = parts[1];
            CommonUses.TryGetValue(symbol, out var use);
            WhereToFinds.TryGetValue(symbol, out var where);
            FunFacts.TryGetValue(symbol, out var fact);
