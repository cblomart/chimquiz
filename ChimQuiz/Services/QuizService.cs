...
public class QuizService(ElementService elementService)
{
    ...
    public QuizSessionState StartNewSession(ISession session, Guid playerId, int playerXp = 0)
    {
        ...
        state.Questions = GenerateQuestions(playerXp);
    ...
    public (QuizQuestionState? question, int index, int total, int combo, int totalXp) GetCurrentQuestion(ISession session)
    {
        ...
        var safeQ = new QuizQuestionState
        {
    ...
    public AnswerResult? SubmitAnswer(ISession session, string answer)
    {
        ...
        if (q.IsTyped)
        {
            ...
            var nc = Normalize(q.CorrectAnswer);
            if (na == nc)
                isCorrect = true;
        ...
        if (isGameOver) state.IsCompleted = true;

        SaveState(session, state);

        // Find the element for info card
        var element = elementService.GetById(q.ElementId);
    ...
    public QuizSessionState? GetState(ISession session) => LoadState(session);
    ...
    /// </summary>
    private static (int maxZ, int nameToSymbol, int symbolToName, int typed) GetDifficulty(int playerXp) => playerXp switch
    {
    ...
    {
        var (maxZ, nameToSymbolCount, symbolToNameCount, symbolTypedCount) = GetDifficulty(playerXp);

        var usedIds   = new HashSet<int>();
        var questions = new List<QuizQuestionState>(TotalQuestions);

        // Name → Symbol (MCQ) — confusable distractors
        for (int i = 0; i < nameToSymbolCount; i++)
        {
            var el = elementService.GetWeightedRandom(usedIds, maxZ);
            usedIds.Add(el.AtomicNumber);
            var wrong   = elementService.GetConfusableSymbols(3, el.Symbol, maxZ);
            var choices = new List<string>(wrong) { el.Symbol };
            Shuffle(choices);
        ...
        for (int i = 0; i < symbolToNameCount; i++)
        {
            var el = elementService.GetWeightedRandom(usedIds, maxZ);
            usedIds.Add(el.AtomicNumber);
            var wrong   = elementService.GetConfusableNames(3, el.Name, maxZ);
            var choices = new List<string>(wrong) { el.Name };
            Shuffle(choices);
            questions.Add(new QuizQuestionState
            {
                ElementId     = el.AtomicNumber,
    ...
    private static int CalculateXp(int combo) => combo switch
    {
        >= 8 => 30,
        >= 5 => 20,
        >= 3 => 15,
    ...
    private static string GetComboMessage(int combo) => combo switch
    {
