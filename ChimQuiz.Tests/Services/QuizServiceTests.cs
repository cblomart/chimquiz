...
namespace ChimQuiz.Tests.Services
{
    public class QuizServiceTests
    {
        ...
        public void StartNewSession_Default_Generates15Questions()
        {
            ...
            QuizSessionState state = _svc.StartNewSession(session, _playerId);
        ...
        public void StartNewSession_CustomCount_GeneratesCorrectQuestions(int count)
        {
            ...
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId, playerXp: 300, questionCount: count);
        ...
        {
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId, questionCount: 1);
        ...
        public void StartNewSession_ClampsAboveMax_UsesMax30()
        {
            ...
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId, playerXp: 300, questionCount: 99);
        ...
        public void StartNewSession_Apprenti_ClampsToPoolSize()
        {
            ...
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId, playerXp: 0, questionCount: 30);
        ...
        public void StartNewSession_AllQuestionsHaveDistinctElements()
        {
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId);
            List<int> ids = state.Questions.Select(q => q.ElementId).ToList();
        ...
        public void GenerateQuestions_Apprenti_ElementsWithinZ20()
        {
            ...
            foreach (QuizQuestionState q in state.Questions)
            {
        ...
        public void GenerateQuestions_Master_ElementsCanExceedZ20()
        {
            ...
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId, playerXp: 5000, questionCount: 15);
        ...
        public void GenerateQuestions_MCQ_HasExactly4Choices()
        {
            ...
            foreach (QuizQuestionState? q in state.Questions.Where(q => !q.IsTyped))
            {
        ...
        public void SubmitAnswer_Correct_EarnsBaseXp()
        {
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId);
            string correctAnswer = state.Questions[0].CorrectAnswer;

            AnswerResult? result = _svc.SubmitAnswer(session, correctAnswer);
        ...
        {
            FakeSession session = NewSession();
            _ = _svc.StartNewSession(session, _playerId);
            string wrongAnswer = "____wrong____";

            AnswerResult? result = _svc.SubmitAnswer(session, wrongAnswer);
        ...
        public void SubmitAnswer_ComboAccumulates()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                string answer = state.Questions[i].CorrectAnswer;
                AnswerResult? result = _svc.SubmitAnswer(session, answer);
                Assert.NotNull(result);
        ...
        public void SubmitAnswer_IncorrectResetsCombo()
        {
            ...
            AnswerResult? result = _svc.SubmitAnswer(session, "____wrong____");
        ...
        {
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId, questionCount: 30);
            ...
            AnswerResult? result = _svc.SubmitAnswer(session, state.Questions[7].CorrectAnswer);
        ...
        public void SubmitAnswer_MaxComboTracked()
        {
            ...
            AnswerResult? result = _svc.SubmitAnswer(session, state.Questions[4].CorrectAnswer);
        ...
        {
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId, questionCount: 5);
        ...
        public void SubmitAnswer_WithWrongAnswers_TriggersRevengeRound()
        {
            ...
            QuizSessionState? reloadedState = _svc.GetState(session);
        ...
        {
            FakeSession session = NewSession();
            _ = _svc.StartNewSession(session, _playerId, questionCount: 30);
        ...
        public void SubmitAnswer_WrongInRevenge_NotAddedToWrongElementIds()
        {
            ...
            QuizSessionState stateBefore = _svc.GetState(session)!;
            int wrongCountBeforeRevenge = stateBefore.WrongElementIds.Count;

            // Fail the revenge question
            _svc.SubmitAnswer(session, "____wrong____");

            QuizSessionState stateAfter = _svc.GetState(session)!;
        ...
        public void CheckAnswer_Typed_AccentInsensitive()
        {
            ...
            QuizSessionState rawState = _svc.GetState(session)!;
            QuizQuestionState? typedQ = rawState.Questions.FirstOrDefault(q => q.IsTyped && q.CorrectAnswer.Any(c => c > 127));
            if (typedQ is null)
            {
                return; // skip if no accented typed question in this run
            }

            // Inject this as the first question by manipulating via a fresh session
            FakeSession session2 = NewSession();
            QuizSessionState state2 = _svc.StartNewSession(session2, _playerId, playerXp: 0, questionCount: 5);
            QuizSessionState rawState2 = _svc.GetState(session2)!;
            QuizQuestionState? typed2 = rawState2.Questions.FirstOrDefault(q => q.IsTyped);
            if (typed2 is null)
            {
                return;
            }
            ...
            if (withoutAccent == typed2.CorrectAnswer)
            {
                return; // skip if no accents to test
            }
            ...
            AnswerResult? result = _svc.SubmitAnswer(session2, withoutAccent);
            Assert.NotNull(result);
        ...
        public void CheckAnswer_Typed_FuzzyMatch_OneTypoAccepted_OnLongWord()
        {
            ...
            QuizQuestionState? typedQ = state.Questions.FirstOrDefault(q =>
            ...
            AnswerResult? result = _svc.SubmitAnswer(session, oneTypo);
            Assert.NotNull(result);
        ...
        {
            FakeSession session = NewSession();
            QuizSessionState state = _svc.StartNewSession(session, _playerId, playerXp: 0, questionCount: 5);

            QuizQuestionState mcqQ = state.Questions.First(q => !q.IsTyped);
            ...
            AnswerResult? result = _svc.SubmitAnswer(session, mcqQ.CorrectAnswer.ToUpperInvariant());
            Assert.NotNull(result);
            Assert.True(result.IsCorrect);
        }
        ...
        {
            FakeSession session = NewSession();
            _ = _svc.StartNewSession(session, _playerId, playerXp: 0, questionCount: 5);
        ...
        public void SubmitAnswer_AfterGameOver_ReturnsNull()
        {
            ...
            AnswerResult? result = _svc.SubmitAnswer(session, "anything");
            Assert.Null(result);
        }
